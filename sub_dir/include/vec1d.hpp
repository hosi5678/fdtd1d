...
// vectorの先頭に値を挿入する
 vec1d& vec1d::push_front(const double d) {
   ...
   // vec1d result(vec.size()+1);  // 現在のオブジェクトをコピーする

   // for(size_t i=0; i<vec.size(); i++) {
   //    result.vec[i]=vec[i];
   // }

   // result.vec[vec.size()]=d;

   // vec=result.vec;

   // return *thisにしないと、e.push_front(d);等で正しい結果にならない。
   return *this;
 }
...
vec1d& vec1d::pop_back(const vec1d& obj) {
   ...
   // vec1d temp(obj.vec.size()-1);

   // for(size_t i=0; i<obj.vec.size()-1; i++){
   //    temp.vec[i]=obj.vec[i];
   // }
...
vec1d& vec1d::pop_front(const vec1d& obj) {
   ...
   // for(size_t i=1; i<obj.vec.size(); i++){
   //    temp.vec[i-1]=obj.vec[i];
   // }
